"""add user.sub column for Auth0 identity

Per the architecture spec, ``email`` is an OPTIONAL claim. Auth0 access
tokens commonly include ``sub`` and the namespaced roles claim, but NOT
``email`` (which lives on the ID token, not the access token). The prior
backend keyed user records on ``email``, so a valid role-bearing access
token without ``email`` would fail user routes.
...
sets ``claims.sub = email``). Going forward:
  ...
  * ``email`` becomes optional metadata; populated when the validator
    ...
    email claim is added to the Action)
