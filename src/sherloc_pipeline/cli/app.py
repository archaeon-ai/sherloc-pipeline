...
def _apply_despike_method_override(despike_method: Optional[DespikeMethod]) -> None:
    ...
    """
    if despike_method is None:
        return
    despike_cfg = get_config().preprocessing.setdefault('despike', {})
    despike_cfg['method'] = despike_method.value
    console.print(f"[cyan]Despike method override: {despike_method.value}[/cyan]")
...
def full_pipeline(
    ...
    despike_method: Optional[DespikeMethod] = typer.Option(
        None, "--despike-method",
        help="Cosmic-ray despike method: 'ml' (certified ML detector), "
             "'modz' (legacy rolling-median), or 'none'. Default: config.yaml (ml).",
    ),
):
...
def plot_cmd(
    ...
    despike_method: Optional[DespikeMethod] = typer.Option(
        None, "--despike-method",
        help="Cosmic-ray despike method: 'ml' (stored masks from the DB; "
             "plot never runs inference), 'modz' (legacy, computed live), "
             "or 'none'. Default: config.yaml (ml). Ignored with --level "
             "(stored levels are not re-processed)."),
...
def process_new_cmd(
    ...
    try:
        ...
        # every mars_target scan plus the cal-target meteorite (SaU 008). The
        # query widens to the mars_target ∪ cal_target candidate set, then
        # is_fittable() — the single-source predicate — selects the fittable
        # scans (SCAN_CLASSIFICATION_SPEC §4.2.2 + Key Decision K6).
        ...
        scans = [
            (sol_num, tgt, scn)
            for sol_num, tgt, scn, ttype in candidate_scans
...
@app.command("backfill-masks")
def backfill_masks_cmd(
    ctx: typer.Context,
    database: Optional[Path] = typer.Option(
        None, "--database", "-d",
        help="Database path (defaults to ./phase.db)"
    ...
    data_dir: Optional[Path] = typer.Option(
        None, "--data-dir",
        help="Loupe workspace root containing sol_XXXX folders (run off NAS). Defaults to config."
    ),
    results_dir: Optional[Path] = typer.Option(
        None, "--results-dir",
        help="Scratch dir for preprocessing artifacts. Defaults to a throwaway temp dir "
             "(masks-only backfill does not write the science results tree)."
    ),
    sol: Optional[str] = typer.Option(
        None, "--sol",
        help="Limit to a single sol (e.g. 0921) — use for the canary rollout."
    ),
    dry_run: bool = typer.Option(False, "--dry-run", help="Show scan count without running backfill"),
    no_engineering: bool = typer.Option(False, "--no-engineering", help="Exclude engineering scans"),
    science: bool = typer.Option(False, "--science", help="Mars targets + cal meteorite (SAU-008) only"),
):
    """Backfill ML cosmic-ray despike masks for already-processed scans.

    Masks-only: runs the certified ML despike detector over each scan's
    normalized spectra and persists the per-(point, region) channel masks to
    the cosmic_ray_masks table — the table the web despike toggle reads. Does
    NOT re-fit: fitted_peaks, mineral IDs, fluorescence groups, and manual
    review decisions are left untouched (preprocessing performs no DB writes;
    the only write is to cosmic_ray_masks). Idempotent — re-running replaces a
    scan's masks for the same method in place.

    Requires the Loupe workspace for each scan to be present under --data-dir,
    and onnxruntime installed (provided by the ml-despike extra).

    The masks produced here are identical to those full-pipeline would persist
    (same certified despike path, same provenance/method string), so the web
    toggle behaves exactly as on a freshly full-pipeline'd scan.

    Examples:
        # Canary: a single sol
        sherloc backfill-masks --sol 0921 --data-dir /path/to/loupe

        # Full mission (science scans only)
        sherloc backfill-masks --science --data-dir /path/to/loupe

        # Count scans without running
    ...
    try:
        ...
        # Discover scans (same selectors as `backfill`), then optionally
        # narrow to a single sol for the canary.
        scans = _iter_all_scans(
            db_path, console,
            exclude_type="engineering" if no_engineering else None,
            science_only=science,
        ...
        if sol is not None:
            sol_int = int(sol)
            scans = [s for s in scans if s[0] == sol_int]

        scope = f", sol {int(sol):04d}" if sol is not None else ""
        console.print(f"[bold]Backfill masks[/bold] (ML despike) — {len(scans)} scan(s){scope}")
        ...
        if not scans:
            console.print("[yellow]No scans matched.[/yellow]")
            raise typer.Exit(code=0)
        ...
        # Preprocessing writes CSV/plot artifacts to a results tree; a
        # masks-only backfill must not disturb the science results, so default
        # to a throwaway scratch dir that is removed afterward.
        scratch = None
        if results_dir is None:
            scratch = tempfile.TemporaryDirectory(prefix="sherloc-backfill-masks-")
            eff_results_dir: Optional[Path] = Path(scratch.name)
        else:
            eff_results_dir = results_dir

        pre_service = PreprocessingService(console=console)
        cr_service = CRMaskService(console=console)
        ...
        try:
            with Progress(
                ...
                task = progress.add_task("Backfilling masks...", total=len(scans))
                for sol_num, tgt, scn in scans:
                    progress.update(task, description=f"Masks: Sol {sol_num} {tgt} {scn}")
                    try:
                        # Preprocess through the certified ML despike step only;
                        # generate_plots=False keeps it lean. No DB writes here.
                        pre_result = pre_service.run_scan(
                            sol=f"{sol_num:04d}", target=tgt or "", scan=scn,
                            data_dir=data_dir,
                            results_dir=eff_results_dir,
                            generate_plots=False,
                            despike_method="ml",
                        )
                        despike_md = (pre_result.metadata or {}).get("despike") or {}
                        if not despike_md.get("masks"):
                            all_errors.append(
                                f"Sol {sol_num} {tgt} {scn}: despike produced no masks"
                            )
                            progress.advance(task)
                            continue
                        cr_result = cr_service.persist_masks(
                            sol=f"{sol_num:04d}", target=tgt or "", scan=scn,
                            despike_metadata=despike_md,
                            database_path=db_path,
                        )
                        meta = cr_result.metadata or {}
                        total_masks += meta.get("masks_inserted", 0)
                        total_skipped += meta.get("regions_skipped", 0)
                        last_method = meta.get("method", last_method)
                        scans_ok += 1
                    except Exception as e:
                        all_errors.append(f"Sol {sol_num} {tgt} {scn}: {e}")
                    progress.advance(task)
        finally:
            if scratch is not None:
                scratch.cleanup()

        method_label = last_method or "ml"
        if json_mode:
            import json as json_mod
            output = CLIResult(
                pipeline_version=_pipeline_version,
                command="backfill-masks",
                result={
                    "scans_matched": len(scans),
                    "scans_persisted": scans_ok,
                    "masks_inserted": total_masks,
                    "regions_skipped": total_skipped,
                    "method": method_label,
                    "errors": all_errors,
                },
            )
            ...
            console.print(f"\n[bold]Backfill masks complete:[/bold]")
            console.print(f"  Scans persisted: {scans_ok}/{len(scans)}")
            console.print(f"  Masks inserted: {total_masks}")
            console.print(f"  Method: {method_label}")
            if total_skipped:
                console.print(f"  Regions skipped (no parent spectrum row): {total_skipped}")
            if all_errors:
                console.print(f"\n[yellow]Errors ({len(all_errors)}):[/yellow]")
                for err in all_errors[:30]:
                    console.print(f"  - {err}")
        ...
        if json_mode:
            ...
            console.print(f"\n[red]Backfill masks failed: {e.message}[/red]")
        sys.exit(e.exit_code)
    except typer.Exit:
        raise
    except Exception as e:
        if json_mode:
            ...
            console.print(f"\n[red]Backfill masks failed: {e}[/red]")
        sys.exit(1)
