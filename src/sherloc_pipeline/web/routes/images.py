...
def select_served_aci(session: Session, scan_id: str) -> Optional[ContextImageORM]:
    ...
    rather than ``.first()``-ing into an angle-range variant. A desync
    between probed row and served row silently lies about Colorized
    button availability when a scan has multiple ACI rows.
