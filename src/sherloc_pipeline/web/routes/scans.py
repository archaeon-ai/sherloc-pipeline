...
def _count_ml_masks(session: Session, scan_id: str) -> int:
    ...
    try:
        return (
            session.query(func.count(CosmicRayMaskORM.id))
        ...
        logger.debug(
            "ml_mask_count query for scan %s failed (%s) — reporting 0",
            scan_id,
            type(exc).__name__,
        )
        return 0
...
def get_scan(request: Request, scan_id: str) -> ScanDetailResponse:
    ...
    detail.ml_mask_count = _count_ml_masks(session, scan_id)
