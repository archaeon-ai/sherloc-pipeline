<script lang="ts">
  ...
  // Auth0 Bearer auth and produced 401s.
  ...
  // flag the renderer would operate on uninitialized refs.
  ...
  // in onMount — without the gate, the reactive statement can fire before
  // onMount when activeImage is non-null at component instantiation.
  //
  ...
  // during the placeholder / auth-required UI state (parent-side clearing
  // alone is not enough).
  ...
  // image props. MapMode.svelte does this on every new
