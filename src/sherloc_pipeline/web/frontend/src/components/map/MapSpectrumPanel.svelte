<script lang="ts">
  ...
  async function fetchClassAverage(domain: string, classId: string, label: string) {
    ...
    try {
      ...
      // helper (regression guard for issue #17: the raw `/api/map/data/...`
      // fetch on this path was the sibling auth-blind defect to the
      // point-spectrum one we just fixed).
      ...
      // fetch produced 401 under Auth0 Bearer-token mode.
