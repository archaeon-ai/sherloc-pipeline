  ...
  async function loadLayerData(
    ...
    for (const [_source, domains] of Object.entries(available)) {
      for (const [domain, info] of Object.entries(domains)) {
        ...
        // Auth-attaching via getMapData() — raw fetch skipped Bearer.
        try {
...
<div class="page-container map-page">
  ...
  <div class="breadcrumb">
    ...
    <button class="btn-link" on:click={() => navigate(`#/scan/${scanId}/workbench`)}>
      {scanLabel || scanId}
    </button>
    <span class="breadcrumb-sep">/</span>
    <span>Map Mode</span>
