...
export interface ScanDetail {
  ...
  // Count of stored ML cosmic-ray despike masks for this scan (issue #6).
  // Drives the Workbench despike method selector's "ML" availability: the
  // option is disabled when this is 0/undefined ("No ML masks stored").
  // Optional + treated as 0 on the SPA side so a legacy backend that omits
  // the field (no `ml_mask_count` in the JSON) keeps ML disabled rather
  // than throwing.
  ml_mask_count?: number;
}
...
export interface SpectrumResponse {
  ...
  // raw spectrum and these stay falsy — the modz result is computed client-
  // side in the chain step, not at the fetch.
  despike_applied?: boolean;
  // Precise applied method string for ML masks, e.g. "ml_v1.3_tau_matched".
  despike_method?: string | null;
  n_masked_channels?: number;
  // ABSOLUTE CCD channel indices (0..2147) replaced on the ml path — point
  // endpoint only. Distinct coordinate system from masked_positions below.
  masked_channels?: number[];
  // Positions INTO the served wavenumber/intensity arrays where the ml
  // despike replaced channels (issue #8). Present on all three spectra
  // endpoints; the Workbench builds the red-triangle spike-marker mask from
  // these in ML mode. Empty/absent on none/modz (modz markers come from the
  // client-side modz compute). For a single region this is the channel's row
  // in that region's wavelength selection; for the R123 composite the served
  // array is the full 2148-stitch so these equal masked_channels.
  masked_positions?: number[];
  // Non-empty when an ML composite (R123) view could not be despiked all-or-
  // none because one or more contributing regions lack a stored mask. The UI
  // surfaces this as a warning rather than silently rendering a mixed view.
  despike_missing_regions?: string[];
  n_uncovered_contributor_channels?: number;
  // Echoed modz parameters when `despike_method=modz` was applied server-side
  // (optional; the Workbench modz path is client-side, so this is typically
  // unused there but typed for completeness against the backend contract).
  despike_params_used?: DespikeParams;
}

// --- Badpix annotation layer (issue #9) ---

/**
 * One known-noisy detector channel for the analyst annotation overlay. Wholly
 * separate from despiking — it never masks or alters spectral values.
 * `position` indexes the served wavenumber/intensity arrays for the requested
 * region (same coordinate system as `masked_positions`); `channel` is the
 * absolute CCD index; `tier` 1 = CR-confusable elevated-noise, 2 = stable hot;
 * `source` ∈ g5_eps | jb25 | both.
 */
export interface BadpixChannel {
  position: number;
  channel: number;
  tier: number;
  source: string;
...
export interface BadpixResponse {
  schema_version: string;
  region: string;
  n_channels: number;
  badpix: BadpixChannel[];
}
...
// Despike method selector (issue #6). Three-way replacement for the legacy
// `despike: bool` on/off semantics:
//   - 'none' — no despike (server returns raw; legacy `despike=false`).
//   - 'ml'   — server applies stored cosmic-ray masks + interpolation; the
//              returned `intensity` IS despiked (legacy `despike=true`).
//   - 'modz' — server returns raw; the rolling-median MAD compute runs
//              client-side in the chain step. Sent so the contract is
//              explicit, but `none` and `modz` are interchangeable at the
//              fetch (both yield a raw spectrum).
export type DespikeMethod = 'none' | 'ml' | 'modz';
...
export interface SpectrumParams {
  ...
  trim_pct?: number;
  // Despike method applied server-side (spec §4.6; issue #6). Supersedes the
  // legacy `despike: bool`. Only `ml` changes the returned array; `none` and
  // `modz` both return raw.
  despike_method?: DespikeMethod;
  // Legacy on/off flag — still accepted by the backend. New code should send
  // `despike_method` instead. Retained for backward compatibility.
  despike?: boolean;
}
...
export interface SubsetRequest {
  ...
  trim_pct?: number;
  despike_method?: DespikeMethod;
  // Legacy on/off flag (see SpectrumParams.despike).
  despike?: boolean;
}
