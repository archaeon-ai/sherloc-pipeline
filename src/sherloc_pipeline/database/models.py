...
class SpectrumORM(Base):
    ...
    cosmic_ray_masks: Mapped[List["CosmicRayMaskORM"]] = relationship(
        "CosmicRayMaskORM",
        back_populates="spectrum",
        cascade="all, delete-orphan",
    )
...
class CosmicRayMaskORM(Base):
    """SQLAlchemy model for CosmicRayMask.

    Stores a persisted cosmic-ray detection mask (satellite of a stored
    ...
    **no** ``updated_at`` column (deliberate). ``to_pydantic`` therefore
    leaves the pydantic model's ``updated_at`` field ``None``.
    """

    __tablename__ = "cosmic_ray_masks"
    ...
    channel_indices: Mapped[list] = mapped_column(JSON, nullable=False)
    ...
    # Relationships
    spectrum: Mapped["SpectrumORM"] = relationship(
        "SpectrumORM", back_populates="cosmic_ray_masks"
    )
    ...
    __table_args__ = (
        ...
        Index("ix_cosmic_ray_masks_spectrum_id", "spectrum_id"),
    )

    def to_pydantic(self) -> CosmicRayMask:
        """Convert to Pydantic CosmicRayMask model.

        The ORM has no ``updated_at`` column (masks are immutable); the
        pydantic model's ``updated_at`` is therefore left as its default
        (``None``).
        """
        return CosmicRayMask(
            id=_str_to_uuid(self.id),
            spectrum_id=_str_to_uuid(self.spectrum_id),
            method=self.method,
    ...
    @classmethod
    def from_pydantic(cls, mask: CosmicRayMask) -> "CosmicRayMaskORM":
        """Create from Pydantic CosmicRayMask model.

        ``updated_at`` is intentionally ignored: this table has no such
        column (masks are immutable, delete+insert on re-run).
        """
        return cls(
            id=_uuid_to_str(mask.id),
            spectrum_id=_uuid_to_str(mask.spectrum_id),
            method=mask.method,
            model_sha256=mask.model_sha256,
            tau=mask.tau,
            channel_indices=list(mask.channel_indices),
