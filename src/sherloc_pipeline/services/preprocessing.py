...
def resolve_despike_method(cli_value: Optional[str], cfg) -> str:
    ...
    Args:
        cli_value: Explicit method override (CLI flag or direct service
            argument), or None to defer to config.
        cfg: Pipeline config object (``preprocessing`` may be a dict or an
            attribute-style object; both layouts occur in this codebase).

    Returns:
        One of ``VALID_DESPIKE_METHODS``.
    """

    def _validate(value: str, source: str) -> str:
        if value not in VALID_DESPIKE_METHODS:
            raise PreprocessingError(
                f"Invalid despike method {value!r} (from {source}). "
                f"Valid values: {', '.join(VALID_DESPIKE_METHODS)}",
                exit_code=1,
                context={"despike_method": value, "source": source},
            )
        return value

    if cli_value is not None:
        return _validate(cli_value, "--despike-method")

    pre = getattr(cfg, 'preprocessing', None)
    if isinstance(pre, dict):
        dsp = pre.get('despike', None)
    else:
        dsp = getattr(pre, 'despike', None) if pre is not None else None
    if isinstance(dsp, dict):
        method = dsp.get('method', None)
    else:
        method = getattr(dsp, 'method', None) if dsp is not None else None

    if method is not None:
        return _validate(method, "config preprocessing.despike.method")
    return DEFAULT_DESPIKE_METHOD
...
class PreprocessingService:
    ...
    def run_scan(
        ...
        Args:
            ...
            despike_method: Despike method override: "ml" (certified ML CR
                detector), "modz" (legacy rolling-median/MAD), or "none"
                (skip the step — the former despike_r1=False behavior).
                If None, resolves via config then the "ml" default
                (resolve_despike_method).
            despike_window: modz window size (odd >=3). If None, uses config default.
            despike_threshold: modz robust z-score threshold. If None, uses config default.
            despike_max_iter: Maximum modz iterations. If None, uses config default.
        ...
        try:
            ...
            with Progress(
                ...
                if effective_despike_method == "ml":
                    ...
                    try:
                        despike_metadata = self._run_ml_despike(
                            ingestion=ingestion,
                            working_dir=working_dir,
                        ...
                        metadata["despike"] = despike_metadata
                        cr_masks_path = (
                            ingestion.get_results_path(target=target, sol=sol, scan=scan)
                            / "cr_masks.json"
                        )
                        if cr_masks_path.exists():
                            artifacts.append(cr_masks_path)
                        progress.update(task3a, description="✅ ML cosmic-ray despiking applied")
                    except PreprocessingError:
                        progress.update(task3a, description="❌ ML cosmic-ray despiking failed")
                        raise
                    except Exception as e:
                        progress.update(task3a, description="❌ ML cosmic-ray despiking failed")
                        error = PreprocessingError(
                            f"Failed to apply ML cosmic-ray despiking: {e}",
                            exit_code=1,
                            context={"working_dir": str(working_dir)}
                        )
                        raise enrich(error, sol=sol, target=target, scan=scan)
                elif effective_despike_method == "modz":
                ...
                try:
                    ...
                    if effective_despike_method == "ml":
                        stats_spectra = {
                            key: frame for key, frame in normalized_spectra.items()
                            if key in ("R1", "fluorescence", "R123")
                        }
                    results = ingestion.generate_statistical_analysis(
                        sol, target, scan, working_dir=working_dir,
                        normalized_spectra=stats_spectra,
                    )
    ...
    def _build_ml_detector(self):
        """Construct the certified ML CR detector (test seam).

        Production resolves the frozen ``DEFAULT_MANIFEST`` artifact via
        fetch-and-cache. Tests monkeypatch this method to inject a
    ...
    def _run_ml_despike(
        ...
        *,
        ingestion,
        working_dir: Path,
        normalized_spectra: Dict[str, pd.DataFrame],
        sol: str,
        target: str,
        scan: str,
        generate_plots: bool,
        ...
        Returns:
            The run-level despike provenance metadata (spec §4.3 step 4):
            method identity, model sha256, per-region taus, installed
            onnxruntime version, per-region flag counts, and the full
            per-point mask sets (channel indices only — no DN values, C5).
        """
        import json
        ...
        try:
            ...
            error = PreprocessingError(
                "despike_method=ml requires activeSpectra.csv and "
                "darkSpectra.csv in the Loupe workspace; re-run with "
                "--despike-method modz or none if raw planes are unavailable",
                exit_code=1,
                context={"working_dir": str(working_dir), "missing": str(e)},
            )
            raise enrich(error, sol=sol, target=target, scan=scan)

        # Step 2: detection scope follows processing scope (§3.4) — a
        # region is featurized/inferred only if a present frame consumes it.
        scope: set = set()
        if 'R1' in normalized_spectra:
            scope.add('R1')
        if 'fluorescence' in normalized_spectra:
            scope.update(('R2', 'R3'))
        if 'R123' in normalized_spectra:
            scope.update(('R1', 'R2', 'R3'))
        regions_in_scope = [r for r in ('R1', 'R2', 'R3') if r in scope]
        if not regions_in_scope:
            raise PreprocessingError(
                "despike_method=ml found no spectral frames to despike",
                exit_code=1,
                context={"working_dir": str(working_dir)},
            )
        ...
        batch_actives, batch_darks, batch_regions, batch_keys = [], [], [], []
        for region in regions_in_scope:
            for point in range(n_points):
                batch_actives.append(planes['active'][region][point])
                batch_darks.append(planes['dark'][region][point])
                batch_regions.append(region)
                batch_keys.append((point, region))
        flagged = detector.detect(batch_actives, batch_darks, batch_regions)

        # masks[region][point] = sorted absolute channel indices
        masks: Dict[str, Dict[int, Any]] = {r: {} for r in regions_in_scope}
        for (point, region), channels in zip(batch_keys, flagged):
            masks[region][point] = channels

        # Step 3: replacement on the frames the chain carries (§3.3 rules).
        manifest = detector.manifest
        windows = dict(manifest.region_windows)
        _, channel_masks = derive_region_channel_masks(
            config, manifest.n_channels
        )

        if 'R1' in normalized_spectra:
            r1_df = normalized_spectra['R1']
            # Export restructured raw normalized R1 (same artifact set as
            # the modz path)
            ingestion.save_spectral_data(
                spectra_df=r1_df,
                target=target,
            ...
            r1_despiked, spike_mask_df = apply_masks_to_r1_frame(
                r1_df, masks, channel_masks['R1'], windows, interpolation_method
            )
            normalized_spectra['R1_despiked'] = r1_despiked
            ...
            if generate_plots:
                ...
                test_plots_dir = ingestion.get_results_path(
                ...
                plot_r1_despike_verification_single(
                    r1_df=r1_df,
                    r1_despiked_df=r1_despiked,
                    spike_mask_df=spike_mask_df,
                    output_dir=test_plots_dir,
                    sol=sol,
                    target=target,
                    scan=scan,
                    point=chosen_point,
                    threshold=float(manifest.tau['R1']),
                )

            # The despiked R1 CSV artifact keeps its legacy name and shape.
            ingestion.save_spectral_data(
                spectra_df=r1_despiked,
                target=target,
                sol=sol,
        ...
        if 'fluorescence' in normalized_spectra:
            normalized_spectra['fluorescence'] = apply_masks_to_fluorescence_frame(
                normalized_spectra['fluorescence'], masks, windows,
                interpolation_method,
            )

        if 'R123' in normalized_spectra:
            winning_map = derive_winning_region_map(
                channel_masks, manifest.n_channels
            )
            normalized_spectra['R123'] = apply_masks_to_r123_frame(
                normalized_spectra['R123'], masks, winning_map, windows,
                interpolation_method,
            )
        ...
        despike_metadata: Dict[str, Any] = {
            ...
            "tau": {r: float(manifest.tau[r]) for r in regions_in_scope},
            "ort_version": detector.ort_version,
            "n_flagged": {
                r: int(sum(len(m) for m in masks[r].values()))
                for r in regions_in_scope
            },
            "masks": {
                str(point): {
                    r: [int(c) for c in masks[r].get(point, [])]
                    for r in regions_in_scope
                }
                for point in range(n_points)
            },
        ...
        results_dir = ingestion.get_results_path(target=target, sol=sol, scan=scan)
        results_dir.mkdir(parents=True, exist_ok=True)
        cr_masks_path = results_dir / "cr_masks.json"
        with open(cr_masks_path, "w") as f:
            json.dump(despike_metadata, f, indent=2)

        return despike_metadata
