...
class CRMaskError(SherlocServiceError):
    """Error during cosmic-ray mask persistence operations.

    This error indicates a failure while persisting or retrieving
    cosmic-ray despike masks (the ``cosmic_ray_masks`` satellite table),
    e.g. a missing database, an unknown scan, or malformed despike
    provenance metadata.

    Example:
        >>> raise CRMaskError("Scan not found in database", exit_code=1)
    """
    pass
