...
@dataclass
class DespikeApplication:
    """Effective despike state for one rendered frame (spec §4.7).

    ``plot`` never runs inference: ``ml`` means stored-mask application
    ...
    producing method is not recorded in the level CSVs, so no method is
    claimed for it.
    """

    method: Optional[str] = None      # effective method: ml | modz | none
    source: str = "none"              # stored_masks | live_modz | none | stored_level
    points_total: int = 0             # point spectra carried by this render
    points_despiked: int = 0          # points actually mask-replaced/despiked
    database_path: Optional[str] = None  # DB consulted for stored masks (ml)
    level: Optional[str] = None       # stored processing level (stored_level)

    def as_metadata(self) -> Dict[str, Any]:
        if self.source == "stored_level":
            # The rendered data's despike state follows from the level
            # name; counts and method claims would misstate provenance.
            return {
                "source": "stored_level",
                "level": self.level,
                "despiked": bool(self.level and "despiked" in self.level),
            }
        out: Dict[str, Any] = {
            "method": self.method,
            "source": self.source,
            "points_total": self.points_total,
            "points_despiked": self.points_despiked,
        }
        if self.database_path is not None:
            out["database_path"] = self.database_path
        return out
...
class SpectralPlotRequest:
    ...
    # (the pre-integration behavior). --level data is never re-processed.
...
class SpectralService:
    ...
    def __init__(
        ...
        *,
        ...
        # Per-invocation despike state ("both" re-enters process(), so the
        # no-mask note aggregates across renders and prints exactly once
        # per top-level invocation — review F8).
        self._despike_depth = 0
        self._despike_no_mask = 0
        self._despike_points_seen = 0
        self._despike_mask_cache: Dict[Tuple[str, str, str], Dict] = {}
    ...
    def process(self, request: SpectralPlotRequest) -> ServiceResult:
        ...
        """
        top_level = self._despike_depth == 0
        if top_level:
            self._despike_no_mask = 0
            self._despike_points_seen = 0
            self._despike_mask_cache = {}

        self._despike_depth += 1
        try:
            # Route based on domain
            if request.domain == "fluor":
                result = self._process_fluor(request)
            elif request.domain == "both":
                result = self._process_both(request)
            # Default: Raman domain (existing behavior)
            elif request.mode == "averaged":
                result = self._process_averaged(request)
            elif request.mode == "subset":
                result = self._process_subset(request)
            else:
                result = self._process_point(request)
        finally:
            self._despike_depth -= 1
        ...
        if top_level and self._despike_no_mask > 0:
            ...
            result.warnings.append(
                f"{self._despike_no_mask} of {self._despike_points_seen} "
                "spectra have no stored CR mask — rendered non-despiked. "
                "Run 'sherloc full-pipeline' with despike method 'ml' to "
                "generate and persist masks, or pass --despike-method "
                "none/modz."
            )
        return result

    # ------------------------------------------------------------------ #
    #  Despike application (spec §4.7 — plot never runs inference)
    # ------------------------------------------------------------------ #

    def _resolve_despike_method(self, request: SpectralPlotRequest) -> str:
        """Effective method for this render: request > config > "ml"."""
        from sherloc_pipeline.services.preprocessing import resolve_despike_method

        return resolve_despike_method(request.despike_method, self.context.config)

    def _despike_interpolation_method(self) -> str:
        """The replacement interpolation the pipeline's despike stage uses."""
        pre = getattr(self.context.config, 'preprocessing', None)
        dsp = pre.get('despike', {}) if isinstance(pre, dict) else {}
        if isinstance(dsp, dict):
            return dsp.get('interpolation_method', 'linear')
        return 'linear'

    def _stored_masks_for_scan(self, request: SpectralPlotRequest) -> Dict:
        """Stored masks for the request's scan, cached per invocation."""
        key = (request.sol, request.target, request.scan)
        if key not in self._despike_mask_cache:
            from sherloc_pipeline.services.cr_masks import CRMaskService

            db_path = Path(os.environ.get("SHERLOC_DB_PATH", "./phase.db"))
            self._despike_mask_cache[key] = {
        ...
        return self._despike_mask_cache[key]

    def _apply_despike(
        self,
        loupe_data: LoupeData,
        request: SpectralPlotRequest,
        kind: Literal["r1", "fluor"],
        points_of_interest: Optional[list[int]] = None,
    ) -> DespikeApplication:
        """Apply the effective despike method to a Loupe-loaded frame.

        Mutates ``loupe_data.spectra_df`` in place. ``ml`` applies stored
        DB masks via the pipeline's own mask-application helpers — no
        inference, ever (spec §4.7); points without a stored mask render
        non-despiked and feed the aggregated note. ``modz`` computes the
        legacy method live on the R1 frame (the legacy method never
        despiked fluorescence — that semantics is preserved). ``none``
        applies nothing.

        Args:
            loupe_data: Loaded Loupe frame (R1 raman or fluorescence sum).
            request: The active request (scan identity + method override).
            kind: Which representation the frame is.
            points_of_interest: The point indices this render actually
                draws (for the summary counts); None means every point
                column in the frame.

        Returns:
            DespikeApplication describing what was applied.
        """
        method = self._resolve_despike_method(request)
        df = loupe_data.spectra_df
        point_cols = [c for c in df.columns if isinstance(c, (int, np.integer))]
        rendered = (
            [p for p in point_cols if p in set(points_of_interest)]
            if points_of_interest is not None
            else list(point_cols)
        )
        state = DespikeApplication(
            method=method, source="none", points_total=len(rendered)
        )

        if method == "none":
            self._print_despike_summary(state, kind)
            return state

        if method == "modz":
            state.source = "live_modz"
            if kind == "r1":
                ...
                despiked, _ = despike_r1_dataframe(df, params)
                loupe_data.spectra_df = despiked
                state.points_despiked = len(rendered)
            # kind == "fluor": the legacy method never despiked the
            # fluorescence representation; points_despiked stays 0.
            self._print_despike_summary(state, kind)
            return state

        # method == "ml": stored masks only (no inference in plot, ever)
        cache = self._stored_masks_for_scan(request)
        masks = cache["masks"]
        state.source = "stored_masks"
        state.database_path = str(cache["database_path"])
        ...
        windows = dict(DEFAULT_MANIFEST.region_windows)
        interpolation_method = self._despike_interpolation_method()

        if kind == "r1":
            covered = set(masks.get("R1", {}))
            if covered:
                _, channel_masks = derive_region_channel_masks(
                    self.context.config, DEFAULT_MANIFEST.n_channels
                )
                despiked, _ = apply_masks_to_r1_frame(
                    df,
                    masks,
                    channel_masks["R1"],
                    windows,
            ...
            # Fluorescence sum: all-or-none per point across contributing
            # regions, mirroring the web composite rule (§4.6) — a point
            # is despiked only if every contributor has a stored record.
            covered = set(masks.get("R2", {})) & set(masks.get("R3", {}))
            if covered:
                filtered = {
                    region: {
                        point: channels
                        for point, channels in masks.get(region, {}).items()
                        if point in covered
                    }
                    for region in ("R2", "R3")
                }
                loupe_data.spectra_df = apply_masks_to_fluorescence_frame(
                    df, filtered, windows, interpolation_method
                )

        state.points_despiked = len([p for p in rendered if p in covered])
        self._despike_points_seen += state.points_total
        self._despike_no_mask += state.points_total - state.points_despiked
        self._print_despike_summary(state, kind)
        return state

    def _print_despike_summary(
        self, state: DespikeApplication, kind: str
    ) -> None:
        """One-line effective-despike-state summary, every render (F8)."""
        if state.method == "none":
            line = "Despike: none"
        elif state.method == "modz" and kind == "fluor":
            line = (
                "Despike: modz — the legacy method does not despike the "
                "fluorescence representation"
            )
        elif state.method == "modz":
            line = (
                f"Despike: modz — legacy despike computed live for "
                f"{state.points_despiked}/{state.points_total} point spectra"
            )
        else:
            line = (
                f"Despike: ml — stored masks applied to "
                f"{state.points_despiked}/{state.points_total} point spectra"
            )
        self.console.print(f"[cyan]{line}[/cyan]", soft_wrap=True)
    ...
    def _process_averaged(self, request: SpectralPlotRequest) -> ServiceResult:
        ...
        # Step 1b: Despike per point spectrum before any averaging
        # (matching pipeline semantics: despike precedes aggregation)
        despike_state = self._apply_despike(loupe_data, request, "r1")
        metadata["despike"] = despike_state.as_metadata()
    ...
    def _process_subset(self, request: SpectralPlotRequest) -> ServiceResult:
        ...
        # Step 2b: Despike the subset's point spectra before averaging
        despike_state = self._apply_despike(subset_data, request, "r1")
        metadata["despike"] = despike_state.as_metadata()
    ...
    def _process_point_from_loupe(self, request: SpectralPlotRequest) -> ServiceResult:
        ...
        # Step 2b: Despike before extraction (the extracted column picks
        # up the mask-replaced/despiked values)
        despike_state = self._apply_despike(
            loupe_data, request, "r1", points_of_interest=[request.point]
        )
        metadata["despike"] = despike_state.as_metadata()
    ...
    def _process_fluor_averaged(
        ...
        # Despike per point spectrum before averaging (fluorescence sum:
        # stored R2/R3 masks, all-or-none per point — spec §3.3/§4.6)
        despike_state = self._apply_despike(
            loupe_data, request, "fluor",
            points_of_interest=request.points if subset else None,
        )
        metadata["despike"] = despike_state.as_metadata()
    ...
    def _process_fluor_point(self, request: SpectralPlotRequest) -> ServiceResult:
        ...
        despike_state = self._apply_despike(
            loupe_data, request, "fluor", points_of_interest=[request.point]
        )
        metadata["despike"] = despike_state.as_metadata()
    ...
    def _process_point_from_results(self, request: SpectralPlotRequest) -> ServiceResult:
        ...
        # --level data is already-processed pipeline output and is never
        # re-processed here (spec §4.7). Its despike state follows from
        # the stored level itself ("normalized" is pre-despike output;
        # "normalized_despiked*" was despiked at processing time by the
        # method of that run, which level CSVs do not record) — no
        # method claim is made. State is still communicated per render:
        # ServiceResult metadata, JSON sidecar, console line.
        despike_state = DespikeApplication(
            source="stored_level",
            level=request.level,
        )
        metadata["despike"] = despike_state.as_metadata()
        if "despiked" in (request.level or ""):
            level_note = "was despiked at processing time"
        else:
            level_note = "is pre-despike stored output"
        self.console.print(
            f"[cyan]Despike: --level data ({request.level}) {level_note}; "
            "not re-processed[/cyan]",
            soft_wrap=True,
        )
    ...
    def _build_metadata(
        ...
        # Cosmic-ray despike state (spec §4.7)
        if despike is not None:
            metadata["despike"] = despike.as_metadata()
