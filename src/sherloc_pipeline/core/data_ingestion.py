...
class DataIngestion:
    ...
    def generate_statistical_analysis(self, sol: str, target: str, scan: str,
                                       ...
                                       normalized_spectra: Optional[Dict[str, pd.DataFrame]] = None) -> Dict[str, int]:
        ...
        Args:
            ...
            normalized_spectra: Pre-processed normalized frames to average.
    ...
    def load_active_dark_planes(self, working_dir: Path) -> Dict[str, Dict[str, np.ndarray]]:
        ...
        Returns:
            ``{"active": {"R1": (n_points, 2148) float ndarray, "R2": ...,
            "R3": ...}, "dark": {...}}``
        ...
        for plane_name, filename in (("active", "activeSpectra.csv"),
                                     ("dark", "darkSpectra.csv")):
            plane_file = working_dir / filename
            require_file(plane_file, f"Raw {plane_name} spectra file not found")

            df = pd.read_csv(plane_file, dtype=str, low_memory=False)
            for col in df.columns:
                if col.startswith(('R1_Channel', 'R2_Channel', 'R3_Channel')):
                    df[col] = pd.to_numeric(df[col], errors='coerce')

            sections = {
                'R1': df.iloc[:n_spectra],
                'R2': df.iloc[n_spectra + 1:2 * n_spectra + 1],
                'R3': df.iloc[2 * n_spectra + 2:3 * n_spectra + 2],
            }
            planes[plane_name] = {}
            for region, section in sections.items():
                values = section.to_numpy(dtype=float)
                if values.shape[0] != n_spectra:
                    raise ValueError(
                        f"{filename} section {region} has {values.shape[0]} "
                        f"rows, expected {n_spectra}"
                    )
                planes[plane_name][region] = values
        return planes
