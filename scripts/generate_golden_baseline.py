...
#: One golden set per despike method ("none" has no golden anchor).
GOLDEN_DIRS = {
    "modz": PROJECT_ROOT / "tests" / "golden" / "sol_921_detail_1",
    "ml": PROJECT_ROOT / "tests" / "golden" / "sol_921_detail_1_ml",
}
...
def main() -> int:
    ...
    parser.add_argument(
        "--despike-method",
        choices=sorted(GOLDEN_DIRS),
        default="ml",
        help="Despike method to anchor (selects the output golden dir).",
    )
    args = parser.parse_args()
    golden_dir = GOLDEN_DIRS[args.despike_method]
    ...
    # Pin the method through the config singleton (the CLI override
    # mechanism) and keep the run hermetic: the ml path produces masks
    # the pipeline would persist into whatever SHERLOC_DB_PATH resolves
    # to — point it at a nonexistent file so no database is touched.
    get_config().preprocessing.setdefault('despike', {})['method'] = (
        args.despike_method
    )
    os.environ["SHERLOC_DB_PATH"] = str(
        Path(tempfile.gettempdir()) / "golden_baseline_no_db.db"
    )
