...
`/etc/passwd`). Patched in v4.1.15 by resolving the candidate path and
asserting it stays inside the dist directory before serving.
...
@pytest.mark.skipif(
    ...
    reason="POSIX-only: exercises the traversal pattern against /etc/passwd",
)
def test_spa_catchall_rejects_traversal_to_etc_passwd(client: TestClient) -> None:
    """Replay of the traversal URL pattern: ten encoded ``..`` segments
    targeting ``/etc/passwd``. The pre-patch handler would
    `os.stat`-resolve this and serve the host's passwd file (HTTP 200,
    text/plain). The patched handler must fall through to the SPA index
    without ever leaking the ``root:x:0:0`` marker."""
    response = client.get(
        "/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/%2e%2e/etc/passwd"
    )
    assert response.status_code == 200
    assert "root:x:0:0" not in response.text, (
        "Traversal pattern leaked /etc/passwd"
    )
    assert "<!doctype html>" in response.text.lower()
